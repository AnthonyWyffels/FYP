...
//this class is everything to do with the finance key. 
public class FinanceClass {

	public static void Main() {
		
		
		System.out.println("bingo made it");
		...
		Main.txtEnter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					String query = "SELECT * FROM financeinput WHERE '"+ str + "' LIKE CONCAT('%',userinput, '%');";  //FROM table don't put from <name of db> not a good idea!!!
					...
					if(rs.next()) 
					{
						...
						String query2 = "SELECT * FROM financeoutput WHERE categorienb="+ categorie + ";";     
					...
					if (Main.txtEnter.getText().contains("change key")) {
						Main.ChooseOption();
					}
